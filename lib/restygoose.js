...
function extractModelName (url, prefixURL, sufixURL) {
	...
	var start = url.indexOf(prefixURL);
	var end = url.indexOf(sufixURL);
	
	if (start >= 0)
		start += prefixURL.length + 1;
	else 
		start = 0;
...
var Restygoose = function(config) {
	...
	if (config.prefixURL === null || config.prefixURL === undefined)
		config.prefixURL = "";

	if (config.sufixURL === null || config.sufixURL === undefined)
		config.sufixURL = "";

	if (config.models === null || config.models === undefined)
		config.models = {};
