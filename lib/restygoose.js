...
var Restygoose = function(config) {
	...
	for (var i  in models) {
		...
		app.get(url, function (req, res) {
			...
			mongooseModel.find(search).exec(function (err, data) {
		...
		console.log('Finding collection dependencies for %s...', model.name);
		for (var key in mongooseModel.schema.paths) {
			if(mongooseModel.schema.paths[key].options.ref) {
				console.log(mongooseModel.schema.paths[key].options.ref);
			}
		}
